...
public class MachineLearningTests : IDisposable
{
    ...
    public MachineLearningTests()
    {
        ...
        // Leave ML:ModelBasePath empty so PricePredictionService falls back to ContentRootPath/MLModels
        _mockConfiguration = new Mock<IConfiguration>();
        _mockConfiguration.Setup(c => c["ML:ModelBasePath"]).Returns(string.Empty);
    ...
    {
        var service = new PricePredictionService(_listingsContext, _mockEnvironment.Object, _mockConfiguration.Object);
        for (int i = 1; i <= 12; i++)
    ...
    {
        var service = new RoommateMatchingService(_roommatesContext, _usersContext);
