...
const RoommateDetailPage: React.FC = () => {
  ...
  if (isLoading) return <Box sx={{ display: 'flex', justifyContent: 'center', py: 8 }}><Typography>{t('common:loading', { defaultValue: 'Učitava se...' })}</Typography></Box>;
  if (!roommate) return <Box sx={{ textAlign: 'center', py: 8 }}><Typography>{t('noPicture')}</Typography></Box>;
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={8}>
          ...
          <Paper sx={{ borderRadius: 3, overflow: 'hidden', mb: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 2, mt: 1, flexWrap: 'wrap' }}>
                    ...
                    {roommate.lifestyle && (
                      <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>
                        {LIFESTYLE_ICONS[roommate.lifestyle]} {t(LIFESTYLE_KEYS[roommate.lifestyle])}
                      </Typography>
                    )}
                    {roommate.workSchedule !== undefined && (
                      <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>
                        {SCHEDULE_ICONS[roommate.workSchedule as number]} {t(SCHEDULE_KEYS[roommate.workSchedule as number])}
                      </Typography>
                    )}
                  </Box>
                </Box>
            ...
            <Box sx={{ display: 'flex', px: 3, py: 1.5, bgcolor: 'primary.main', gap: 3, flexWrap: 'wrap' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, color: '#fff' }}>
                ...
                <Box>
                  <Typography variant="caption" sx={{ opacity: 0.7, display: 'block', lineHeight: 1 }}>{t('availableFromLabel')}</Typography>
                  <Typography variant="body2" fontWeight="bold">
                    {availableFrom === 'Odmah'
                      ? <span style={{ color: '#a5d6a7' }}>✓ {t('availableNow')}</span>
                      : availableFrom}
                  </Typography>
                </Box>
              </Box>
              {(roommate.budgetMin || roommate.budgetMax) && (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, color: '#fff' }}>
                  ...
                  <Box>
                    <Typography variant="caption" sx={{ opacity: 0.7, display: 'block', lineHeight: 1 }}>{t('budget')}</Typography>
                    <Typography variant="body2" fontWeight="bold">
                      {roommate.budgetMin && roommate.budgetMax
                        ? `€${roommate.budgetMin} – €${roommate.budgetMax}/${t('monthAbbr')}`
                        : roommate.budgetMin ? `od €${roommate.budgetMin}/${t('monthAbbr')}` : `do €${roommate.budgetMax}/${t('monthAbbr')}`}
                    </Typography>
                  </Box>
                </Box>
              )}
              {(roommate.minimumStayMonths || roommate.maximumStayMonths) && (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, color: '#fff' }}>
                  <Box>
                    <Typography variant="caption" sx={{ opacity: 0.7, display: 'block', lineHeight: 1 }}>{t('periodOfStay')}</Typography>
                    <Typography variant="body2" fontWeight="bold">
                      {roommate.minimumStayMonths && roommate.maximumStayMonths
                        ? `${roommate.minimumStayMonths}–${roommate.maximumStayMonths} ${t('monthAbbr')}`
                        : roommate.minimumStayMonths ? `min. ${roommate.minimumStayMonths} ${t('monthAbbr')}` : `max. ${roommate.maximumStayMonths} ${t('monthAbbr')}`}
                    </Typography>
                  </Box>
                </Box>
              )}
          ...
          {roommate.bio && (
            <Paper sx={{ p: 3, mb: 3, borderRadius: 3 }}>
              <Typography variant="h6" fontWeight="bold" gutterBottom>{t('aboutMe')}</Typography>
          ...
          {(hobbies.length > 0 || languages.length > 0) && (
            <Paper sx={{ p: 3, mb: 3, borderRadius: 3 }}>
              {hobbies.length > 0 && (
                <>
                  <Typography variant="subtitle1" fontWeight="bold" gutterBottom>{t('hobbiesAndInterests')}</Typography>
              ...
              {languages.length > 0 && (
                <>
                  <Typography variant="subtitle1" fontWeight="bold" gutterBottom>{t('languages')}</Typography>
          ...
          <Paper sx={{ p: 3, mb: 3, borderRadius: 3 }}>
            <Typography variant="h6" fontWeight="bold" gutterBottom>{t('lifestyleAndRules')}</Typography>
            <Grid container spacing={0}>
              <Grid item xs={12} sm={6}>
                <PrefRow label={t('smokingPref')} value={roommate.smokingAllowed} trueLabel={t('smokerLabel')} falseLabel={t('nonSmokerLabel')} />
                <PrefRow label={t('petsPref')} value={roommate.petFriendly} trueLabel={t('prefYesOk')} falseLabel={t('prefNo')} />
                <PrefRow label={t('guestsPref')} value={roommate.guestsAllowed} trueLabel={t('prefYesOk')} falseLabel={t('prefNo')} />
                <PrefRow label={t('musicPref')} value={roommate.musicFriendly} trueLabel={t('prefYesOk')} falseLabel={t('prefNo')} />
              </Grid>
              <Grid item xs={12} sm={6}>
                {roommate.cleanliness && (
                  <PrefRow label={t('cleanlinessPref')} value={CLEANLINESS_KEYS[roommate.cleanliness] ? t(CLEANLINESS_KEYS[roommate.cleanliness]) : roommate.cleanliness} />
                )}
                {roommate.lifestyle && (
                  <PrefRow label={t('lifestylePref')} value={`${LIFESTYLE_ICONS[roommate.lifestyle]} ${t(LIFESTYLE_KEYS[roommate.lifestyle])}`} />
                )}
                {roommate.workSchedule !== undefined && (
                  <PrefRow label={t('rhythmPref')} value={`${SCHEDULE_ICONS[roommate.workSchedule as number]} ${t(SCHEDULE_KEYS[roommate.workSchedule as number])}`} />
                )}
              </Grid>
            </Grid>
          </Paper>
          ...
          {(roommate.lookingForRoomType || roommate.lookingForApartmentType || roommate.preferredLocation) && (
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              <Typography variant="h6" fontWeight="bold" gutterBottom>{t('lookingForSection')}</Typography>
              {roommate.preferredLocation && (
              ...
              {roommate.lookingForRoomType && (
                <Box sx={{ mb: 0.5 }}>
                  <Typography component="span" variant="body2" color="text.secondary">{t('typeLabel')}</Typography>
        ...
        <Grid item xs={12} md={4}>
          <Box sx={{ position: 'sticky', top: 90, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Card sx={{ borderRadius: 3 }}>
              <CardContent>
                {!isOwn ? (
                  ...
                  <>
                    <Typography variant="subtitle2" fontWeight="bold" gutterBottom>{t('profileCompleteness')}</Typography>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                      <Typography variant="body2" color="text.secondary">{t('profileFilledLabel')}</Typography>
                      <Typography variant="body2" fontWeight="bold"
            ...
            <Card sx={{ borderRadius: 3 }}>
              <CardContent>
                <Typography variant="subtitle2" fontWeight="bold" gutterBottom>{t('quickOverview')}</Typography>
                <Divider sx={{ mb: 1.5 }} />
                {availableFrom !== 'Odmah' && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.6 }}>
                    <Typography variant="body2" color="text.secondary">{t('availableFromLabel')}</Typography>
                    <Typography variant="body2" fontWeight="medium">{availableFrom}</Typography>
                  </Box>
                )}
                {availableFrom === 'Odmah' && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.6 }}>
                    <Typography variant="body2" color="text.secondary">{t('moveIn')}</Typography>
                    <Typography variant="body2" fontWeight="medium" color="success.main">{t('availableNowFull')}</Typography>
                  </Box>
                )}
                {roommate.budgetIncludes && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.6 }}>
                    <Typography variant="body2" color="text.secondary">{t('includedInPrice')}</Typography>
                ...
                {roommate.minimumStayMonths && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.6 }}>
                    <Typography variant="body2" color="text.secondary">{t('minPeriod')}</Typography>
                    <Typography variant="body2" fontWeight="medium">{roommate.minimumStayMonths} {t('monthAbbr')}</Typography>
                  </Box>
                )}
              </CardContent>
            </Card>
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)}>
        ...
        <DialogContent>
          <Typography>{t('deleteProfileConfirm')}</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteOpen(false)}>{t('common:cancel', { defaultValue: 'Odustani' })}</Button>
